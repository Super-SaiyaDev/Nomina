/* i'm creating the views of the bd */
USE NominaSystem;
...
SELECT U.IdUsers, 
       ...
       U.Clave, 
       U.activo,
       R.IdRole AS IDrole, R.NameRole, R.Descriptions,
       P.NamePermission, P.Descriptions AS DP
FROM Users AS U
INNER JOIN Roles AS R
ON R.IDrole = U.IdRole
INNER JOIN Permissions AS P
ON P.IdRole = R.IDrole
...
-- -------------------------------- for Search a user --------------------------
DELIMITER //
CREATE PROCEDURE SearchUsers(IN P_IdUsers INT)
BEGIN
...
DELIMITER //
CREATE PROCEDURE CreateUsers(IN P_NameUser VARCHAR(40), IN  P_LastName VARCHAR(50), IN P_Users VARCHAR(80) ,
                             IN P_Clave VARCHAR(255), IN P_IdRole INT )
BEGIN
...
-- ----------------------------------------- for Update a user -------------------------------------------------
DELIMITER //
CREATE PROCEDURE UpdateUsers( IN P_IdUser INT, IN P_NameUser VARCHAR(40), IN  P_LastName VARCHAR(50), IN P_Users VARCHAR(80) ,
							 IN P_Clave VARCHAR(255), IN P_IdRole INT )
BEGIN
...
CALL UpdateUsers(2, "Pedro","Vergaras Solis", 'PedroVS', '123456', 1);


-- ----------------------------------------- for Delete a user -------------------------------------------------
DELIMITER //
CREATE PROCEDURE DeleteUsers( IN P_IdUser INT)
BEGIN
...
DELIMITER //
CREATE PROCEDURE LoginUsers( IN P_Users VARCHAR(80), IN P_Clave VARCHAR(255))
BEGIN
